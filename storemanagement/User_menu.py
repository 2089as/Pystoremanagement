...
class User:
    ...
    def makeprint(self):
       if messagebox.askyesno("Alert!", "Print this transaction?") == True:
        wb = Workbook()
        ws = wb.active
        ws.append(["Invoice No.", "Date", "Time", "Product ID", "Product Name", "Quantity", "Price"])
        for item in self.tree.get_children():
            values = self.tree.item(item)["values"]
            ws.append([self.invoice, datetime.datetime.now().strftime("%d-%m-%Y"), datetime.datetime.now().strftime("%H:%M:%S"), values[1], values[2], values[3], values[4]])
        ws.append(["Total", self.total.get()])
        wb.save("transaction.xlsx")
        messagebox.showinfo("Success", "Transaction printed successfully!")
